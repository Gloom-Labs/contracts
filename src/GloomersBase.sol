...
contract GloomersBase is ERC721AOffsetable, Ownable {
    ...
    uint256 public constant TOKEN_ID_OFFSET = 3334;
    ...
    uint256 public immutable randomSeed = 1350; // temp while using foundry
    ...
    constructor() ERC721AOffsetable("Gloomers", "GLOOMERS", 3334) Ownable(msg.sender) {
    ...
    function mint(uint256 quantity) external payable {
        ...
        // Calculate spotMintQuantity only if the total with quantity exceeds the randomSeed
        if (totalWithQuantity > _sequentialUpTo()) {
            uint256 availableForSpotMint = totalWithQuantity - _sequentialUpTo();
            spotMintQuantity = availableForSpotMint > quantity ? quantity : availableForSpotMint;
    ...
    function nextSpotMintTokenId() public view returns (uint256) {
        return _nextTokenId() + _totalSpotMinted() + 1;
    }
