 ...
 */
contract ERC721AOffsetable is IERC721A {
    ...
    // Token offset for sequential mints.
    ...
    constructor(string memory name_, string memory symbol_, uint256 tokenOffset_) {
        _name = name_;
        _symbol = symbol_;
        _currentIndex = tokenOffset_;
        _tokenOffset = tokenOffset_;

        if (_sequentialUpTo() < tokenOffset_) _revert(SequentialUpToTooSmall.selector);
    }
